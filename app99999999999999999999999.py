...
DEBUG_MAT_QTY = 100
SWORD_SELL_MULTIPLIER = 0.20 # 업그레이드 가격의 20% 이익을 보고 판매
...
def get_sell_price(current_level):
    ...
    # 다음 레벨의 업그레이드 비용을 기준으로 20% 이익을 붙여 판매
...
def enhance_sword_core(use_protection=False):
    """강화의 핵심 로직을 실행합니다. (성공/실패 판정 및 결과 처리)"""
    current_level = st.session_state.level # 실패 후 재시도 시 failed_level이 들어있음
    req = ENHANCE_REQUIREMENTS.get(current_level)
    success_rate = req['rate']
    
    # 4. 강화 성공/실패 판정 (실패했으므로 이 로직은 실패 후 처리 로직임)
    
    # 실패 로직
    fail_message = f"💥 **[강화 실패]** 무기가 파괴됩니다. (확률: {success_rate * 100:.1f}%)"
...
def handle_protection_choice(choice):
    ...
    # 실패 레벨을 현재 레벨로 임시 설정 (로직 처리를 위해)
    st.session_state.level = failed_level
    
    if choice == 'yes':
        # 방지권 사용 로직 실행 (Level 6 이상에서 파괴 방지 및 레벨 유지)
        enhance_sword_core(use_protection=True)
    else:
        # 방지권 미사용 로직 실행 (파괴 또는 레벨 하락)
        enhance_sword_core(use_protection=False)
    
    st.session_state.use_protection_prompt = False # ⭐ UI 제거를 위해 False로 설정

    # 강화 메시지를 표시하기 위해 Rerun
...
def main():
    ...
    st.title("🔥 검 강화하기 시뮬레이터 (판매/방지권 프롬프트)")
    st.markdown("---")
    ...
    if st.session_state.use_protection_prompt:
        ...
        st.markdown("---")
        # 프롬프트가 떠 있을 때는 강화 및 판매 버튼을 더 이상 렌더링하지 않고 종료
        return 
    ...
    if current_level < len(ENHANCE_REQUIREMENTS) - 1:
        ...
        with action_col2:
            sell_price = get_sell_price(current_level)
            if st.button(f"💰 무기 판매 ({sell_price:,} Gold 획득)", use_container_width=True):
                sell_sword()
