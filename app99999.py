...
INITIAL_MONEY = 300000  # 시작 소지금 (초기 플레이를 위해 넉넉하게 설정)
INITIAL_LEVEL = 0
SWORD_NAMES = {
...
ENHANCE_REQUIREMENTS = {
    ...
    4: {'name': SWORD_NAMES[5], 'rate': 0.90, 'gold': 2000, 'mat_qty': 1, 'mat_name': '알 수 없는 재료A', 'item_req': None, 'note': None},
    5: {'name': SWORD_NAMES[6], 'rate': 0.86, 'gold': 6000, 'mat_qty': 1, 'mat_name': '알 수 없는 재료A', 'item_req': None, 'note': None},
    6: {'name': SWORD_NAMES[7], 'rate': 0.81, 'gold': 15000, 'mat_qty': 1, 'mat_name': '국적불분명 철조각', 'item_req': None, 'note': None},
    7: {'name': SWORD_NAMES[8], 'rate': 0.75, 'gold': 25000, 'mat_qty': 1, 'mat_name': '국적불분명 철조각', 'item_req': None, 'note': None},
    8: {'name': SWORD_NAMES[9], 'rate': 0.70, 'gold': 50000, 'mat_qty': 1, 'mat_name': '국적불분명 철조각', 'item_req': None, 'note': None},
    9: {'name': SWORD_NAMES[10], 'rate': 0.66, 'gold': 90000, 'mat_qty': 1, 'mat_name': '타우의 뼈 부스러기', 'item_req': None, 'note': None},
    10: {'name': SWORD_NAMES[11], 'rate': 0.62, 'gold': 180000, 'mat_qty': 1, 'mat_name': '빛 바랜 형광물질', 'item_req': None, 'note': None},
    11: {'name': SWORD_NAMES[12], 'rate': 0.61, 'gold': 500000, 'mat_qty': 1, 'mat_name': '스위스산 철조각', 'item_req': None, 'note': None},
    12: {'name': SWORD_NAMES[13], 'rate': 0.54, 'gold': 1000000, 'mat_qty': 1, 'mat_name': '스위스산 철조각', 'item_req': None, 'note': None},
    13: {'name': SWORD_NAMES[14], 'rate': 0.50, 'gold': 2000000, 'mat_qty': 2, 'mat_name': '불꽃마검 손잡이', 'item_req': None, 'note': None},
    14: {'name': SWORD_NAMES[15], 'rate': 0.49, 'gold': 5000000, 'mat_qty': 3, 'mat_name': '사악한 영혼', 'item_req': None, 'note': None},
    15: {'name': SWORD_NAMES[16], 'rate': 0.46, 'gold': 10000000, 'mat_qty': 4, 'mat_name': '도끼 가루', 'item_req': None, 'note': None},
    16: {'name': SWORD_NAMES[17], 'rate': 0.44, 'gold': 20000000, 'mat_qty': 7, 'mat_name': '투명 물질', 'item_req': None, 'note': None},
    17: {'name': SWORD_NAMES[18], 'rate': 0.40, 'gold': 44500000, 'mat_qty': 9, 'mat_name': '알 수 없는 재료B', 'item_req': None, 'note': None}, 
    18: {'name': SWORD_NAMES[19], 'rate': 0.38, 'gold': 72000000, 'mat_qty': 10, 'mat_name': '알 수 없는 재료B', 'item_req': None, 'note': '보관필요'}, 
    19: {'name': SWORD_NAMES[20], 'rate': 0.35, 'gold': 120000000, 'mat_qty': 12, 'mat_name': '알 수 없는 재료B', 'item_req': None, 'note': '보관필요'}, 
    20: {'name': SWORD_NAMES[21], 'rate': 0.33, 'gold': 240000000, 'mat_qty': 15, 'mat_name': '알 수 없는 재료B', 'item_req': None, 'note': None}, 
...
# 상점 아이템 및 가격
STORE_ITEMS = {
    '파괴 방지권': 10000000 # 1000만 Gold로 설정 (고가 아이템에 대비)
}

MONEY_PER_CLICK = 100000  # 돈 벌기 버튼 클릭당 획득 금액
...
def enhance_sword(use_protection=False):
    ...
    if not req:
        st.session_state.message = "⚠️ **[에러]** 다음 레벨 강화 정보를 찾을 수 없습니다."
        return
    ...
    item_req_str = req['item_req']
    protection_impossible = '파괴방지권 불가' in (mat_name or '')
    ...
    if mat_name and '불가' not in mat_name and st.session_state.inventory.get(mat_name, 0) < mat_qty:
        st.session_state.message = f"🚨 **[강화 실패]** 재료 **{mat_name}** ({mat_qty}개)가 부족합니다."
        return

    if item_req_str:
        item_req = parse_item_requirement(item_req_str)
        if item_req and st.session_state.inventory.get(item_req['name'], 0) < item_req['qty']:
             st.session_state.message = f"🚨 **[강화 실패]** 재료 **{item_req['name']}** ({item_req['qty']}개)가 부족합니다."
             return
        
    # 3. 비용 차감 (실패 여부와 관계없이 소모)
    ...
    if random.random() < success_rate:
        ...
        # 이전 검 소모
        prev_name = SWORD_NAMES[current_level]
        if prev_name in st.session_state.inventory:
            st.session_state.inventory[prev_name] = max(0, st.session_state.inventory[prev_name] - 1)
        ...
        if current_level >= 6: 
            ...
            # 파괴된 검 인벤토리에서 제거
            prev_name = SWORD_NAMES[current_level].replace('[보관필요]', '').replace('[하드버그]', '').strip()
            if prev_name in st.session_state.inventory:
                 st.session_state.inventory[prev_name] = max(0, st.session_state.inventory[prev_name] - 1)
...
def earn_money():
    """돈 벌기 버튼 클릭 시 돈을 증가시킵니다."""
    st.session_state.money += MONEY_PER_CLICK
    st.session_state.message = f"💰 {MONEY_PER_CLICK:,} Gold를 벌었습니다. 강화를 위해 더 많은 돈이 필요합니다!"
...
def buy_item(item_name, price):
    ...
    if st.session_state.money < price:
        st.session_state.message = f"🚨 **[구매 실패]** {item_name}을(를) 구매하기에는 돈이 부족합니다. ({price:,} Gold 필요)"
        return
...
def main():
    ...
    st.set_page_config(page_title="검 강화하기 시뮬레이터 (파괴/방지권 시스템)", layout="wide")
    st.title("🔥 검 강화하기 시뮬레이터 (파괴/방지권 시스템)")
    st.markdown("---")
    ...
    if current_level < len(ENHANCE_REQUIREMENTS):
        ...
        if req['mat_name'] and req['mat_qty'] > 0:
        ...
        with protection_col:
            ...
            if not protection_impossible and st.session_state.inventory.get('파괴 방지권', 0) > 0:
                use_protection = st.checkbox(f"🛡️ **파괴 방지권 사용** (보유: {st.session_state.inventory.get('파괴 방지권', 0)}개)", value=False)
                 ...
                 st.caption("파괴 방지권이 없거나 상점에서 구매 가능합니다.")
    ...
    with inv_col:
        display_data = []
        # 모든 가능한 아이템/재료를 포함하여 0이 아닌 것만 표시
